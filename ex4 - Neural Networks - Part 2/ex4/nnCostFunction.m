...
% transform y from a 5000 x 1 vector with values 1,...,10
%  to a 5000 x 10 matrix with all 0 and a 1 at the index of correct value
yVec = zeros(m,num_labels);
for i=1:m
    yVec(i,y(i))= 1;
end;

% remove the bias terms of the Thetas for the regularization
Theta1WOBias = Theta1( :, 2:size(Theta1,2) );
Theta2WOBias = Theta2( :, 2:size(Theta2,2) );

% calculate cost function
J = sum(  1/m * sum( -yVec.*log(h)-(1-yVec).*log(1-h) )  ) ...
    + lambda/(2*m) * (  sum( sum(Theta1WOBias.^2) ) + sum( sum(Theta2WOBias.^2) )  );             
...
for t = 1:m
    ...
    delta2 = delta2(2:end, :);
    % Accumulate the gradient from this example using the formula
    Theta1_grad = Theta1_grad + delta2 * a1';
    Theta2_grad = Theta2_grad + delta3 * a2';
end;
% Obtain the gradient for the neural network cost function with
% regularization
